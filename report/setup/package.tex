...
% 只在 pdflatex 模式下加载 CJKpunct 和 CJKutf8
...
\def\atemp{pdflatex}\ifx\atemp\usewhat
\usepackage{CJKutf8}                        % 用在UTF8编码环境下，它可以自动调用CJK，同时针对UTF8编码作了设置（仅 pdflatex 模式）
\fi
...
% 生成有书签的 pdf 及其生成方式。通常可以在 tjumain.tex 文件的第一行选择 pdflatex 或者是 xelatex 编译手段。如果选择前者，则使用 pdflatex + pdflatex 编译; 如果选择后者，在编译的时候使用 xelatex + xelatex 编译。出现混淆的时候，系统会报错。
